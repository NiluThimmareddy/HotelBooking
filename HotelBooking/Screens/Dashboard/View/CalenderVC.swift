...
//  HotelBooking
//
//  Created by toqsoft on 02/06/25.
//
//
...
class CalenderVC: UIViewController {
    ...
    var selectedAdults: Int?
    var selectedChildren: Int?
    ...
    override func viewDidLoad() {
        ...
        segmentControl.setTitleTextAttributes([.foregroundColor: UIColor(hex: "#003B95")], for: .normal)
...
//            let dateRangeText = "\(startText) - \(endText)"
...
//    }
    @IBAction func selectDatesButtonAction(_ sender: Any) {
        ...
        var roomInfo = ""
        if let rooms = selectedRooms, let adults = selectedAdults {
            roomInfo = " (\(rooms) Room\(rooms > 1 ? "s" : ""), \(adults) Adult\(adults > 1 ? "s" : "")"
            if let children = selectedChildren, children > 0 {
                roomInfo += ", \(children) Child\(children > 1 ? "ren" : "")"
            }
            roomInfo += ")"
        }
        ...
        if let start = startDate, let end = endDate {
            let startText = formatter.string(from: start)
            let endText = formatter.string(from: end)
            let dateRangeText = "\(startText) - \(endText)\(roomInfo)"
            delegate?.didSelectDateRange(dateRangeText)
        } else if let start = startDate {
            let startText = formatter.string(from: start)
            let dateRangeText = "\(startText)\(roomInfo)"
            delegate?.didSelectDateRange(dateRangeText)
        } else {
            let defaultDateRange = Date.todayAndTomorrowFormattedRange()
            let dateRangeText = "\(defaultDateRange)\(roomInfo)"
            delegate?.didSelectDateRange(dateRangeText)
        }
